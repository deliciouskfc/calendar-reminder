...
- **UI**：登录/注册用旧锁屏 overlay（#lockOverlay）+ .lock-card 样式；新增 .auth-tabs/.auth-tab 选项卡、.account-bar（更新日志弹窗内 👤 邮箱 + ⏻ 退出登录按钮，登录时才显示）；unlock 屏有「退出登录，切换账号」；课表工具栏 `#tutorialBtn`（❓，始终可见）→ `openTutorial()` 弹出使用教程弹窗（10 个章节：加课/退课/拖动/撤销/多课表/课件/云同步/校历/账户/检查更新）
